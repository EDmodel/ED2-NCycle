...
subroutine update_phenology(doy, cpoly, isi, lat)
   ...
   patchloop: do ipa = 1,csite%npatches
      ...
      cohortloop: do ico = 1,cpatch%ncohorts
            ...
            if (theta(kroot) < theta_crit) then
               ...
               if (cpatch%phenology_status(ico) < 2) then
                  ...
                  csite%fsc_in_no_excess(ipa) = csite%fsc_in_no_excess(ipa)                                    &
                                    + cpatch%nplant(ico) * cpatch%leaf_drop(ico)           &
                                    * f_labile(ipft) / c2n_leaf(ipft) !JL!
         ...
         !JL!
               !   cpatch%nstorage(ico) = cpatch%nstorage(ico) + (cpatch%bleaf(ico)          &
...
!JL!
               !   cpatch%nstorage(ico) = cpatch%nstorage(ico)                              &
                !                       + (retained_carbon_fraction * delta_bleaf)/c2n_storage
!JL!
                 ...
                 ! csite%fsn_in(ipa)     = csite%fsn_in(ipa)                                &
                  !                      + delta_bleaf * cpatch%nplant(ico)                 &
                   !                     * retained_carbon_fraction                         &
                    !                    * (1.0 / c2n_leaf(ipft) - 1.0/c2n_storage)
                !JL Turned this off because when bstorage changes (above) this does not affect 
                !the nstorage term. fsn_in was already updated for the leaf loss
            ...
            if (delta_bleaf > 0.0 .and. cpatch%phenology_status(ico) < 2) then
              ...
              ! and going into Nstorage.F_labile is set to  1
               ...
               cpatch%bstorage(ico) = cpatch%bstorage(ico) + retained_carbon_fraction      &
                                    * delta_bleaf
               !JL!   0.48% of N from dropped leaves    
               cpatch%nstorage(ico) = cpatch%nstorage(ico) + 0.48                          &
               ...
               ! different.                                                                !
               !---------------------------------------------------------------------------!
               ! JL N from leaf drop is added to fsn_in and nstorage separately above.
               ! This part of the code was created when nstorage was not a separate pool
               ! and instead based on bstorage.

...
subroutine ed_1st_master (ipara, nnodestotal,nslaves, headnode_num, name_name, &
   ...
   include 'mpif.h'
   !----- Arguments. ----------------------------------------------------------------------!
   integer, intent(in) :: my_ens, my_real, my_lat, my_lon
   ...
   call read_nl(trim(name_name),my_real,my_ens,my_lat,my_lon)
   ...
   call read_met_driver_head(my_ens,my_real)

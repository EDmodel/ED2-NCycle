...
subroutine ed_output(analysis_time,new_day,dail_analy_time,mont_analy_time,dcyc_analy_time &
   ...
   if ( analysis_time .or.   history_time .or. dcycle_time  .or.                           &
      ...
      !else                                                                !JL
      !   do ifm=1,ngrids
      !   call integrate_precipitation(edgrid_g(ifm))
      !   end do
      end if
   end if
   !---------------------------------------------------------------------------------------!
   ...
   if (new_day .and. (writing_dail .or. writing_mont .or. writing_dcyc)) then
    ...
    !elseif (new_day) then !JL!
    !    do ifm=1,ngrids
    !     call normalize_precipitation(edgrid_g(ifm))
    !     call zero_precipitation(edgrid_g(ifm))
    !    enddo
   end if
   !---------------------------------------------------------------------------------------!
         ...
         siteloop: do isi=1,cpoly%nsites
            ...
            patchloop: do ipa=1,csite%npatches
               ...
               cohortloop: do ico = 1,cpatch%ncohorts
                ...
                !                     * cpatch%bstorage(ico) * cpatch%nplant(ico)           &     !should i add ns
                 !                    / c2n_storage * area_pa ! C:N not pft specific  !JL!
